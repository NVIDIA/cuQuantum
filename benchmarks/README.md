...
For the `cudaq` frontend, the `--reuse` flag (enabled by default) will reuse the JIT-compiled circuit from warmup runs for benchmarking, so the JIT compilation time will not be measured as part of the execution time. Use the `--no-reuse` flag to disable this behavior.
