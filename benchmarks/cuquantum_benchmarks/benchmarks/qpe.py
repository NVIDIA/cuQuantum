...
class QPE(Benchmark):
    ...
    def generateGatesSequence(nqubits, config):
        ...
        if measure:
            ...
            circuit.append(('measure', [list(range(nqubits-in_nqubits))]))
        return circuit
    ...
    def _make_qpe_component(in_qubits, t_qubits, U, unfold, U_name):
        ...
        # 1. Setup Eigenstate
        component.append(('x', [in_qubits[0]]))
        # 2. Superposition Counting Qubits
        all_h = [('h', [idx]) for idx in t_qubits]
        component += all_h
        # 3. Controlled-U
        ...
        for t in range(t_nqubits):
            if unfold:
                for i in range(2 ** t):
                    component.append(('cu', [U, f'{U_name}', t_qubits[t], in_qubits]))

            else:
                new_U = prev_U @ U
                component.append(('cu', [new_U, f'{U_name}^(2^{t})', t_qubits[t_nqubits - t - 1], in_qubits]))
                prev_U = new_U
