...
int main()
{
    ...
    **********************/
    ...
    floatType *outputActivation_h = (floatType*) malloc(sizeof(floatType) * elementsVec[numInputs]);
    if (outputActivation_h == NULL)
    {
        printf("Error: Host memory allocation failed!\n");
        return -1;
    }

    void* gradientsOut_d[numInputs] = {nullptr};
    for (auto i : gradInputIDs)
    {
        HANDLE_CUDA_ERROR( cudaMalloc((void**) &gradientsOut_d[i], sizeVec[i]) );
    }
    void* gradientsOut_h[numInputs] = {nullptr};
    for (auto i : gradInputIDs)
    {
        gradientsOut_h[i] = (floatType*) malloc(sizeVec[i]);
        if (gradientsOut_h[i] == NULL)
        {
           printf("Error: Host memory allocation failed!\n");
           return -1;
        }
    ...
    *******************/
    ...
    for (size_t e = 0; e < elementsVec[numInputs]; ++e)
        outputActivation_h[e] = (floatType) 1.0;

    for (int i = 0; i < numInputs; ++i)
    {
        HANDLE_CUDA_ERROR( cudaMemcpy(rawDataIn_d[i], rawDataIn_h[i], sizeVec[i], cudaMemcpyHostToDevice) );
    }
    HANDLE_CUDA_ERROR( cudaMemcpy(outputActivation_d, outputActivation_h, sizeVec[numInputs], cudaMemcpyHostToDevice) );
    ...
    *******************************/
    ...
    /*******************************
    * Set input tensor ids that requrie gradient
    *******************************/

    cutensornetTensorIDList_t tensorIDList {
        .numTensors = static_cast<int32_t>(gradInputIDs.size()),
        .data = gradInputIDs.data()
    };
    ...
    HANDLE_ERROR(cutensornetNetworkSetAttribute(handle,
                                                ...
                                                &tensorIDList,
                                                sizeof(cutensornetTensorIDList_t)));
   ...
   HANDLE_ERROR( cutensornetCreateSliceGroupFromIDRange(handle, 0, numSlices, 1, &sliceGroup) );
    ...
    for (int i = 0; i < numRuns; ++i)
    {
        ...
        */
        ...
        HANDLE_ERROR( cutensornetComputeGradientsBackward(handle,
                        plan,
                        rawDataIn_d,
                        outputActivation_d, 
                        gradientsOut_d,
        ...
        // Purge the cache to make room for the next run to use cache memory
        HANDLE_ERROR( cutensornetWorkspacePurgeCache(handle, workDesc, CUTENSORNET_MEMSPACE_DEVICE) );
    ...
    if(verbose)
        printf("Contracted the tensor network and computed gradients\n");

    HANDLE_CUDA_ERROR( cudaMemcpy(O_h, O_d, sizeVec[numInputs], cudaMemcpyDeviceToHost) ); // restore the output tensor on Host
    
    for (auto i : gradInputIDs)
    {
        HANDLE_CUDA_ERROR( cudaMemcpy(gradientsOut_h[i], gradientsOut_d[i], sizeVec[i], cudaMemcpyDeviceToHost) );
    }
    ...
    ****************/
    ...
    if (O_h) free(O_h);
    if (outputActivation_h) free(outputActivation_h);
    for (int i = 0; i < numInputs; ++i)
    {
        if (rawDataIn_h[i]) 
            free(rawDataIn_h[i]);
        if (gradientsOut_h[i]) 
            free(gradientsOut_h[i]);
    }
    // Free GPU memory resources
    if (workScratch) cudaFree(workScratch);
    if (workCache) cudaFree(workCache);
    if (O_d) cudaFree(O_d);
    if (outputActivation_d) cudaFree(outputActivation_d);
    for (int i = 0; i < numInputs; ++i)
    {
        if (rawDataIn_d[i]) 
            cudaFree(rawDataIn_d[i]);
        if (gradientsOut_d[i]) 
            cudaFree(gradientsOut_d[i]);
    }
    if(verbose)
