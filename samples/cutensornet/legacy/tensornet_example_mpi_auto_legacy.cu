...
int main(int argc, char** argv)
{
    ...
    for (int t = 0; t < numInputs; ++t)
    {
        ...
        // strides are optional; if no stride is provided, cuTensorNet assumes a generalized column-major data layout
        stridesIn[t] = NULL;
    }

    // Set up tensor network
    cutensornetNetworkDescriptor_t networkDesc;
    HANDLE_ERROR(cutensornetCreateNetworkDescriptor(handle, numInputs, numModesIn, extentsIn, stridesIn, modesIn, NULL,
                                                    tensorModes[numInputs].size(), tensorExtents[numInputs].data(),
                                                    /*stridesOut = */ NULL, tensorModes[numInputs].data(),
