    ...
    def test_norm(self):
        ...
        for engine in self.target_engines:
            for _ in get_engine_iters(engine):
                ...
                message = f"{engine.__class__.__name__} maxDiff={abs(norm1-norm2)}, tol={engine.tolerance}"
    ...
    def test_state_vector(self):
        ...
        for engine in self.target_engines:
            for _ in get_engine_iters(engine):
                ...
                message = f"{engine.__class__.__name__} maxDiff={abs(sv1-sv2).max()}, tol={engine.tolerance}"
    ...
    def test_amplitude(self):
        for bitstring in bitstring_generator(self.n_qubits, self.num_tests_per_task):    
            ...
            for engine in self.target_engines:
                ...
                message = f"{engine.__class__.__name__} diff={abs(amp1-amp2)}, tol={engine.tolerance}"
        ...
        for fixed in where_fixed_generator(self.qubits, self.num_fix_sites_max):
            ...
            for engine in self.target_engines:
                for _ in get_engine_iters(engine):
                    ...
                    message = f"{engine.__class__.__name__} maxDiff={abs(batched_amps1-batched_amps2).max()}, tol={engine.tolerance}"
            ...
            for engine in self.target_engines:
                ...
                for kwargs in all_kwargs:
                    for _ in get_engine_iters(engine):
                        ...
                        message = f"{engine.__class__.__name__} maxDiff={abs(rdm1-rdm2).max()} tol={engine.tolerance}"
