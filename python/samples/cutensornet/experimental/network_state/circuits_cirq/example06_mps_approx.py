...
with NetworkState.from_circuit(circuit, dtype='complex128', backend='cupy', config=config) as state:
    ...
    # Note that there is low entanglement from the default cirq.testing.random_circuit protocal and for this particular example, 
    # we will see the final output shape lower than `max_extent`
