...
if rank == root:
    ...
    A = np.empty(np.prod(extent_A), dtype=np.float64)
    B = np.empty(np.prod(extent_B), dtype=np.float64)
    C = np.empty(np.prod(extent_C), dtype=np.float64)
    D = np.empty(np.prod(extent_D), dtype=np.float64)
...
C_d = cp.asarray(C)
D_d = cp.asarray(D)
R_d = cp.empty(np.prod(extent_R), dtype=np.float64)
